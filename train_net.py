...
"""
# --- numpy alias hotfix for numpy>=1.24 ---
import numpy as np
for _name, _alias in {
    "bool": bool,
    "int": int,
...
class Trainer(DefaultTrainer):
    ...
    """
    @classmethod
    def create_ddp_model(cls, model, *, device_ids=None, broadcast_buffers=False):
        return DDP(
            model,
...
def setup(args):
    ...
    add_s2_corr_config(cfg)

    # 在 merge 之前声明 DDP 节点（若你在 YAML 中写了 DDP.*，可被识别）
    cfg.DDP = CN()
    cfg.DDP.FIND_UNUSED_PARAMETERS = False   # 默认值
    cfg.DDP.BROADCAST_BUFFERS = False        # 建议 False
    ...
    # 在 freeze 之前覆盖（可按需保留）
    cfg.DDP.FIND_UNUSED_PARAMETERS = True
    cfg.DDP.BROADCAST_BUFFERS = False
